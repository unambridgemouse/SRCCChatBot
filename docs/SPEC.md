...
- **Rentio（レンティオ）**：家電レンタルサービス。体験店舗が遠方の場合の代替案
- **囲碁将棋チャンネル**：主要な認知経路（CMの放送媒体）。直販価格の優遇対象
- **製品名表記**：SenseRobot / センスロボット / 囲碁ロボット は同一製品。回答内では「センスロボット」または「囲碁ロボット」を使用する
...
LLMのSSEトークンは「数文字の塊が不規則な間隔で」到着するため、受信即描画ではカクついて見える。
そこで `ChatWindow.tsx` で **受信と描画を分離** している。

| 変数 | 役割 |
|------|------|
| `target` | 受信済みの全文（SSE受信時はここに溜めるだけ） |
| `shown` | 表示済み文字数（16ms間隔のタイマーで加算） |

- 基本速度 `TYPE_BASE_CPS`（55文字/秒）で1文字ずつ描画する。
- 未表示分（backlog）に比例した加速項 `TYPE_CATCHUP_GAIN` を加え、生成が速いときは表示も速くなる。
  → **滑らかにするが遅くはしない**（全体の完了時間は受信速度に追随する）。
- 受信完了後、描き切ってから `isStreaming` を解除し、引用元・画像・メタデータを確定表示する。
- タブ非アクティブ時の一気出しを防ぐため、1tickで進める時間は `TYPE_MAX_DT`（0.25秒）で頭打ち。

速度の調整は `ChatWindow.tsx` 冒頭の定数のみで完結する。

#### 速度の上限について（Ver 1.2.1 の実測結果）

**表示はトークンの到着速度を超えられない。** `TYPE_BASE_CPS` を上げても、到着より速くなった時点で
「到着即描画」になるだけで、速くならずカクつきだけが戻る。

Ver 1.2.1 で `TYPE_BASE_CPS` を 55 → 95（1.7倍）に上げて実測したが、**体感速度は変わらなかった**。
これは描画速度が律速ではないことの裏付けであり、Ver 1.2.2 で 55 に戻した。

→ **律速はトークンの到着速度／TTFT（最初の1文字が出るまでの待ち）側にある。**
体感をさらに速くするにはバックエンド側の対策が必要。

| 手段 | 効果 |
|------|------|
| 回答の短文化（プロンプト調整） | 総文字数が減るため最も効く |
| TTFT短縮（Entity抽出・Pinecone・Cohere rerankの直列処理を短縮／並列化） | 最初の1文字までの「待ち」が減る |
| 回答生成モデルの変更（Sonnet→Haiku） | 生成は速いが精度とのトレードオフ。設計方針（回答生成は精度優先）に反する |
