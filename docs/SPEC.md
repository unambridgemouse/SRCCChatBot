...
- 基本速度 `TYPE_BASE_CPS`（95文字/秒）で1文字ずつ描画する。
...
**速度の上限について**: 表示はトークンの到着速度を超えられない。
`TYPE_BASE_CPS` を到着速度より大きく上げても「到着即描画」になるだけで、
速くならずカクつきだけが戻る。体感をさらに速くするにはバックエンド側の対策が要る。

| 手段 | 効果 |
|------|------|
| 回答の短文化（プロンプト調整） | 総文字数が減るため最も効く |
| TTFT短縮（Entity抽出・Pinecone・Cohere rerankの直列処理を短縮） | 最初の1文字が出るまでの「待ち」が減る |
| 回答生成モデルの変更（Sonnet→Haiku） | 生成は速いが精度とのトレードオフ |
