...
_LOCATION_WORDS = ["場所", "ところ", "どこ", "店", "ショップ", "県", "都", "道", "府", "オンライン"]
...
def is_prefecture_only_query(query: str) -> bool:
    """クエリが都道府県名のみ（例: 「神奈川」「神奈川県」）の場合 True を返す。"""
    q = query.strip()
    if q in PREFECTURES:
        return True
    # 「神奈川県」「東京都」「大阪府」→ 末尾1文字を除いて照合（"道"は北海道がそのままリストにあるため除外）
    for suffix in ["県", "都", "府"]:
        if q.endswith(suffix) and q[:-1] in PREFECTURES:
            return True
    return False


def extract_prefecture(query: str) -> str | None:
    """クエリから都道府県名を抽出して返す。見つからない場合は None。"""
    q = query.strip()
    if q in PREFECTURES:
        return q
    for suffix in ["県", "都", "府"]:
...
def needs_location_clarification(query: str) -> bool:
    """体験系クエリで都道府県名が指定されていない場合 True を返す。
    「場所はある？」「どこで？」は地名が未確定なので clarification 対象。
    都道府県名が含まれている場合のみ直接店舗案内へ進む。
    購入系は対象外。
    """
    if not any(w in query for w in _EXPERIENCE_WORDS):
        return False
    return not any(p in query for p in PREFECTURES)
